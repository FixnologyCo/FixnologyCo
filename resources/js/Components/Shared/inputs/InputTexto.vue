...
<template>
  <div class="w-full">
    <div class="contador-label flex items-center justify-between">
      <label class="my-[5px] text-[14px]" 
      :class="[
      model.length >= 1 ? 'text-semaforo-verde' : '',
      error ? 'text-semaforo-rojo' : ''
      ]">{{ label }}</label>
      <p class="text-[10px] " :class="[model.length === maxLength ? 'text-semaforo-rojo' : 'text-secundary-light']">
        {{ (model || "").length }} / {{ maxLength }}
      </p>
    </div>

    <div :class="[inputContainerClass, model.length >= 1 ? 'border-semaforo-verde' : 'border-secundary-light']">
      <span
      ...
      <input
        ...
        v-model="model"
        @blur="emit('blur')"
        @input="emit('input', $event)"3
        class="w-full outline-none font-normal bg-transparent dark:text-mono-blanco placeholder:text-[14px]"
      />
    </div>

    <span v-if="error" :class="[model.length >= 1 ? 'hidden' : '']" class="text-sm flex items-center mt-1 gap-1 text-universal-naranja">
