  ...
  >
    <aside class="relative h-full">
      ...
      <div class="tienda rounded-md border px-2 py-1 mt-3 flex items-center gap-2">
        <div
          class="user h-[20px] w-[20px] rounded-full overflow-hidden flex items-center justify-center"
          :class="[bgClase]"
        ></div>
      ...
      <div class="navegacion mt-5">
        <ul class="flex flex-col gap-1">
          ...
          <div
            ...
            class="text-center text-secundary-light text-[10px]"
          >
            Gestión
          </div>
            ...
            <a :href="route('aplicacion.misAplicaciones', { aplicacion, rol })">
              <span class="flex items-center justify-between">
                <div class="flex items-center gap-1">
                  ...
                  <span v-if="sidebarExpandido" class="text-[14px]"
                    >Mis aplicaciones</span
                  >
                </div>
                <div
                  :class="[currentRoute === aplicacionesFixRoute ? focus : hover]"
                ></div>
              </span>
            </a>
          </li>
          <li
            :class="[currentRoute === usuariosRoute ? [bgOpacity] : 'bg-transparent']"
            class="px-2 py-1.5 rounded-[5px] cursor-pointer"
          >
